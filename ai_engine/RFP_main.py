...
def save_json(merged_chunks: List[dict], out_path: str):
    # Remove orig_chunk before saving (it's not JSON serializable)
    clean_chunks = []
    for chunk in merged_chunks:
        clean_chunk = {k: v for k, v in chunk.items() if k != 'orig_chunk'}
        clean_chunks.append(clean_chunk)
    
    with open(out_path, "w", encoding="utf-8") as jf:
        json.dump(clean_chunks, jf, indent=2, ensure_ascii=False)
    print(f"✅ JSON saved: {out_path}")

# -----------------------
# Main callable function (FIXED)
# -----------------------
def process_pdf(pdf_path: str, ocr: bool = False):
    """
    Process PDF and return both file paths AND chunk data.
    
    Args:
        pdf_path: Path to PDF file
        ocr: Whether to use OCR (currently unused but kept for compatibility)
    
    Returns:
        tuple: (txt_file_path, json_file_path, merged_chunks_data)
    """
    tokenizer = AutoTokenizer.from_pretrained(MODEL_ID)
    chunk_dicts, _ = chunk_document(pdf_path, tokenizer)
    
    # Remove orig_chunk before merging (not needed and causes issues)
    for chunk in chunk_dicts:
        chunk.pop('orig_chunk', None)
    
    merged = merge_small_chunks_forward(chunk_dicts, tokenizer)

    input_stem = Path(pdf_path).stem.replace(" ", "")
    txt_out = f"{input_stem}_chunks.txt"
    json_out = f"{input_stem}_chunks.json"

    save_txt(merged, txt_out)
    save_json(merged, json_out)

    # Return file paths AND the actual data
    return txt_out, json_out, merged